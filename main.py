...
def parse_args():
    ...
    parser.add_argument('--image_size',
                        default=256,
                        type=int)
    parser.add_argument('--max_prot_steps',
                        default=60, # Keep previous default, now as max
                        type=int,
                        help="Maximum number of protection optimization steps per image.")
    ...
    parser.add_argument('--use_lpips_loss',
                        action='store_true', # Use like: --use_lpips_loss
                        help="Use LPIPS perceptual loss for structural guidance instead of/in addition to self-attention loss.")
    parser.add_argument('--lpips_weight',
                        default=0.5, # Tune this value
                        type=float,
                        help="Weight for the LPIPS structural loss.")
    parser.add_argument('--replace_attn_loss',
                        action='store_true', # Use like: --replace_attn_loss
                        help="If using LPIPS, replace the self-attention loss entirely. Otherwise, augment it.")
    parser.add_argument('--early_stop_success_criteria',
                        action='store_true', # Use like: --early_stop_success_criteria
                        help="Enable early stopping for an image once it meets the success criteria against the test model.")
    parser.add_argument('--early_stop_far_level',
                        default=0.01,
                        type=float,
                        choices=[0.1, 0.01, 0.001],
                        help='FAR threshold level (0.1, 0.01, 0.001) to use for the early stopping check.')
